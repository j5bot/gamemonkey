// ==UserScript==
// @name         BGG Last Play Date
// @namespace    https://github.com/j5bot/gamemonkey
// @version      0.1
// @description  Add the date of the last play to collection views which already contain play counts
// @author       j5bot
...
(function () {
    ...
    const playsSelector = '[id^="quickplay_results"] a:not([data-play-date="true"])';
    const playParamRegEx = new RegExp('/plays/(.+?)/(\\d+)', 'i');

    const addPlayDateToCell = item => {
        if (!item) {
            return;
        }

        const { link, playDate } = item;

        if (!playDate) {
            console.error('no play date found');
            return;
        }

        const cell = link.parentElement;
        const playDateElement = document.createElement('div');
        playDateElement.textContent = playDate;
        cell.appendChild(playDateElement);
    };
    ...
    const dequeue = async (delay = 5000) => {
        ...
        fetchPlayDate(item).then(item => {
            addPlayDateToCell(item);
            setTimeout(() => dequeue(delay), delay);
        });
    };
    ...
    const fetchPlayDate = async item => {
        return new Promise((resolve, reject) => {
            ...
            const onload = (response) => {
                if (response.status === 429) {
                    ...
                    setTimeout(() => fetchPlayDate(item).then(resolve).catch(reject), defaultDelay);
                    return;
                }
                if (response.status === 200) {
                    resolve({
                        ...item,
                        playDate: getLatestPlayDate(response.responseText),
                    });
                    return;
                }
    ...
    const init = async (rawPlaysLinks) => {
        ...
        playLinks.forEach(link => {
            link.setAttribute('data-play-date', 'true');
    ...
    if (unsafeWindow.gamemonkey) {
        console.info('Last play date script init via gamemonkey base script...');
